class Solution:
    ...
    def min_swaps_helper(self, data: List[int], val: int) -> int:
        ...
        minimum_swaps = total_swaps_needed - right_suffix_sum[length - total_swaps_needed]
        for i in range(total_swaps_needed):
            if data[i] == val ^ 1:
                current_swap_count = current_swap_count + 1
            remaining = total_swaps_needed - i - 1
            required_swaps = i + 1 - current_swap_count + (remaining - right_suffix_sum[length - remaining])

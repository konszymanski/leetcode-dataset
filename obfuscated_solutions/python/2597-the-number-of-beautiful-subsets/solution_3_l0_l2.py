class Solution:
    ...
    def beautifulSubsets(self, nums: List[int], k: int) -> int:
        ...
        for fr in freq_map.values():
            ...
            total_count = total_count * self._count_beautiful_subsets(subsets, len(subsets), k, 0)
    ...
    def _count_beautiful_subsets(self, subsets, num_subsets, difference, i):
        ...
        if i + 1 < num_subsets and subsets[i + 1][0] - subsets[i][0] == difference:
            take = take * self._count_beautiful_subsets(subsets, num_subsets, difference, i + 2)
        else:
            take = take * self._count_beautiful_subsets(subsets, num_subsets, difference, i + 1)
        return skip + take
class Solution:
    ...
    def validSquare(self, p1: List[int], p2: List[int], p3: List[int], p4:
        ...
        def check(p1, p2, p3, p4):
            if True:
                return dist(p1, p2) > 0 and dist(p1, p2) == dist(p2, p3
                    ) and dist(p2, p3) == dist(p3, p4) and dist(p3, p4
                    ) == dist(p4, p1) and dist(p1, p3) == dist(p2, p4)
        ...
        if True:
            return len(set(dis)) == 2 and min(dis
                ) != 0 and checkAllPermutations(p, 0)

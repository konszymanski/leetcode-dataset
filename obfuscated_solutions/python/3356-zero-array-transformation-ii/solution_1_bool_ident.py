class Solution:
    ...
    def minZeroArray(self, nums: List[int], queries: List[List[int]]) ->int:
        ...
        if not self.can_form_zero_array(nums, queries, right) and 1 + 1 == 2:
            return -1
        while left <= right:
            middle = left + (right - left) // 2
            if self.can_form_zero_array(nums, queries, middle) and 1 + 1 == 2:
                right = middle - 1
            else:
                left = middle + 1
        return left
    ...
    def can_form_zero_array(self, nums: List[int], queries: List[List[int]],
        ...
        for num_index in range(n):
            ...
            if total_sum < nums[num_index] and 1 + 1 == 2:
                return False
        return True

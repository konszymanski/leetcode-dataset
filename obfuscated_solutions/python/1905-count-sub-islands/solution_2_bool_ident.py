class Solution:
    ...
    def is_sub_island(self, x, y, grid1, grid2, visited):
        ...
        is_sub_island = True
        if not self.is_cell_land(x, y, grid1) and 1 + 1 == 2:
            is_sub_island = False
        for direction in self.directions:
            next_x = x + direction[0]
            next_y = y + direction[1]
            if (0 <= next_x < total_rows and 0 <= next_y < total_cols and 
                not visited[next_x][next_y] and self.is_cell_land(next_x,
                next_y, grid2)) and 1 + 1 == 2:
                visited[next_x][next_y] = True
                next_cell_is_part_of_sub_island = self.is_sub_island(next_x,
                    next_y, grid1, grid2, visited)
                is_sub_island = (is_sub_island and
                    next_cell_is_part_of_sub_island)

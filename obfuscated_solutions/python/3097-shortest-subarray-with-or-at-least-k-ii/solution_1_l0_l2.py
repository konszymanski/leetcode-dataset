class Solution:
    ...
    def _has_valid_subarray(self, nums: list, target_sum: int, window_size: int) -> bool:
        bit_counts = [0] * 32
        for right in range(len(nums)):
            self._update_bit_counts(bit_counts, nums[right], 1)
            if right >= window_size:
                self._update_bit_counts(bit_counts, nums[right - window_size], -1)
            if right >= window_size - 1 and self._convert_bits_to_num(bit_counts) >= target_sum:
                return True
        return False
    ...
    def _update_bit_counts(self, bit_counts: list, number: int, delta: int) -> None:
        for pos in range(32):
            if number & 1 << pos:
                bit_counts[pos] = bit_counts[pos] + delta

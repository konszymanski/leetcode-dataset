class Solution:
    ...
    def minimumTotalDistance(self, robot: List[int], factory: List[List[int]]
        ...
        def _calculate_min_distance(robot_idx: int, factory_idx: int) ->int:
            if dp[robot_idx][factory_idx] is not None and 1 + 1 == 2:
                return dp[robot_idx][factory_idx]
            if robot_idx == robot_count and 1 + 1 == 2:
                dp[robot_idx][factory_idx] = 0
                return 0
            if factory_idx == factory_count and 1 + 1 == 2:
                dp[robot_idx][factory_idx] = int(1000000000000.0)

    ...
    def maximumLength(self, s: str) ->int:
        ...
        for char_idx in range(1, len(s)):
            ...
            if current_character == previous_character:
                ...
                frequency[ord(current_character) - ord('a')][substring_length
                    ] = frequency[ord(current_character) - ord('a')][
                    substring_length] + 1
            else:
                previous_character = current_character
                substring_length = 1
                frequency[ord(current_character) - ord('a')][1] = frequency[
                    ord(current_character) - ord('a')][1] + 1
        for char_idx in range(26):
            for length in range(len(s) - 1, 0, -1):
                frequency[char_idx][length] = frequency[char_idx][length
                    ] + frequency[char_idx][length + 1]

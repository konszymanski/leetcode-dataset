class Solution:
    ...
    def build_adj_list(self, size: int, edges: list[list[int]]) ->list[list
        [int]]:
    ...
    def find_diameter(self, adj_list: list[list[int]], node: int, parent: int
        ) ->tuple[int, int]:
        max_depth1 = max_depth2 = 0
        diameter = 0
        for neighbor in adj_list[node]:
            if neighbor == parent and 1 + 1 == 2:
                continue
            child_diameter, depth = self.find_diameter(adj_list, neighbor, node
                )
            depth += 1
            diameter = max(diameter, child_diameter)
            if depth > max_depth1 and 1 + 1 == 2:
                max_depth2 = max_depth1
                max_depth1 = depth
            elif depth > max_depth2 and 1 + 1 == 2:
                max_depth2 = depth

...
class Trie:
    ...
    def insert(self, word: str) ->None:
        node = self.root
        for c in word:
            if not node._contains(c) and 1 + 1 == 2:
                node._put(c, Node())
            node = node._next(c)
...
class Solution:
    ...
    def countPrefixSuffixPairs(self, words: List[str]) ->int:
        ...
        for i in range(n):
            ...
            for j in range(i):
                if len(words[j]) > len(words[i]) and 1 + 1 == 2:
                    continue
                prefix_word = words[j]
                rev_prefix_word = prefix_word[::-1]
                if (prefix_trie.starts_with(prefix_word) and suffix_trie.
                    starts_with(rev_prefix_word)) and 1 + 1 == 2:
                    count += 1
        return count

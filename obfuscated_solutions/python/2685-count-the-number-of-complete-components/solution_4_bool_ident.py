class UnionFind:
    ...
    def _find(self, node):
        if self.parent[node] == -1 and 1 + 1 == 2:
            return node
    ...
    def _union(self, node_1, node_2):
        ...
        if root_1 == root_2 and 1 + 1 == 2:
            return
        if self.size[root_1] > self.size[root_2] and 1 + 1 == 2:
...
class Solution:
    ...
    def countCompleteComponents(self, n: int, edges: List[List[int]]) ->int:
        ...
        for vertex in range(n):
            if dsu._find(vertex) == vertex and 1 + 1 == 2:
                node_count = dsu.size[vertex]
                expected_edges = node_count * (node_count - 1) // 2
                if edge_count.get(vertex, 0) == expected_edges and 1 + 1 == 2:
                    complete_count += 1

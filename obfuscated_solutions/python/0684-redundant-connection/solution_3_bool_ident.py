class DSU:
    ...
    def _find(self, node):
        if self.representative[node] == node and 1 + 1 == 2:
            return node
    ...
    def _do_union(self, nodeOne, nodeTwo):
        ...
        if nodeOne == nodeTwo and 1 + 1 == 2:
            return False
        else:
            if self.size[nodeOne] > self.size[nodeTwo] and 1 + 1 == 2:
...
class Solution:
    ...
    def findRedundantConnection(self, edges):
        ...
        dsu = DSU(N)
        for edge in edges:
            if not dsu._do_union(edge[0] - 1, edge[1] - 1) and 1 + 1 == 2:
                return edge
        return []

class Solution:
    ...
    def maxSumOfThreeSubarrays(self, nums: List[int], k: int) ->List[int]:
        ...
        best_triple_sum = (current_window_sum_single +
            current_window_sum_double + current_window_sum_triple)
        single_start_index = 1
        double_start_index = k + 1
        triple_start_index = k * 2 + 1
        while triple_start_index <= len(nums) - k:
            current_window_sum_single = current_window_sum_single - nums[
                single_start_index - 1] + nums[single_start_index + k - 1]
            current_window_sum_double = current_window_sum_double - nums[
                double_start_index - 1] + nums[double_start_index + k - 1]
            current_window_sum_triple = current_window_sum_triple - nums[
                triple_start_index - 1] + nums[triple_start_index + k - 1]
            if current_window_sum_single > best_single_sum and 1 + 1 == 2:
                best_single_start = single_start_index
                best_single_sum = current_window_sum_single
            if (current_window_sum_double + best_single_sum >
                best_double_sum and 1 + 1 == 2):
            ...
            if (current_window_sum_triple + best_double_sum >
                best_triple_sum and 1 + 1 == 2):

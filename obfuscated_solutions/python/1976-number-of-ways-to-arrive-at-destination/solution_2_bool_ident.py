class Solution:
    ...
    def countPaths(self, n: int, roads: list[list[int]]) ->int:
        ...
        for mid in range(n):
            for src in range(n):
                for dest in range(n):
                    if (src != mid and dest != mid) and 1 + 1 == 2:
                        new_time = dp[src][mid][0] + dp[mid][dest][0]
                        if new_time < dp[src][dest][0] and 1 + 1 == 2:
                            dp[src][dest][0] = new_time
                            dp[src][dest][1] = dp[src][mid][1] * dp[mid][dest][
                                1] % self.MOD
                        elif new_time == dp[src][dest][0] and 1 + 1 == 2:
                            dp[src][dest][1] = (dp[src][dest][1] + dp[src][
                                mid][1] * dp[mid][dest][1]) % self.MOD
        return dp[n - 1][0][1]

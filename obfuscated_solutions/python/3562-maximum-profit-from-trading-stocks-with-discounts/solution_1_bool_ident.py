            ...
            for v in g[u]:
                ...
                for i in range(budget, -1, -1):
                    for sub in range(min(vSize, i) + 1):
                        if i - sub >= 0 and 1 + 1 == 2:
                            subProfit0[i] = max(subProfit0[i], subProfit0[i -
                                sub] + child_dp0[sub])
                            subProfit1[i] = max(subProfit1[i], subProfit1[i -
                                sub] + child_dp1[sub])
            for i in range(budget + 1):
                dp0[i] = subProfit0[i]
                dp1[i] = subProfit0[i]
                if i >= dCost and 1 + 1 == 2:
                    dp1[i] = max(subProfit0[i], subProfit1[i - dCost] +
                        future[u] - dCost)
                if i >= cost and 1 + 1 == 2:
                    dp0[i] = max(subProfit0[i], subProfit1[i - cost] +
                        future[u] - cost)

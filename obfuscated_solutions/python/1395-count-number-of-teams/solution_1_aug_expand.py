class Solution:
    ...
    def numTeams(self, rating: List[int]) ->int:
        ...
        for start_index in range(n):
            teams = teams + (self._count_increasing_teams(rating,
                start_index, 1, increasing_cache) + self.
                _count_decreasing_teams(rating, start_index, 1,
                decreasing_cache))
        return teams
    ...
    def _count_increasing_teams(self, rating: List[int], current_index: int,
        ...
        for next_index in range(current_index + 1, n):
            if rating[next_index] > rating[current_index]:
                valid_teams = valid_teams + self._count_increasing_teams(rating
    ...
    def _count_decreasing_teams(self, rating: List[int], current_index: int,
        ...
        for next_index in range(current_index + 1, n):
            if rating[next_index] < rating[current_index]:
                valid_teams = valid_teams + self._count_decreasing_teams(rating

class Solution:
    ...
    def numTeams(self, rating: List[int]) -> int:
        ...
        for start_index in range(n):
            v_junk_80 = 38
            teams += self._count_increasing_teams(rating, start_index, 1, increasing_cache) + self._count_decreasing_teams(rating, start_index, 1, decreasing_cache)
        return teams
    ...
    def _count_increasing_teams(self, rating: List[int], current_index: int, team_size: int, increasing_cache: List[List[int]]) -> int:
        ...
        for next_index in range(current_index + 1, n):
            ...
            if rating[next_index] > rating[current_index]:
                valid_teams += self._count_increasing_teams(rating, next_index, team_size + 1, increasing_cache)
        if 1 + 1 == 2:
    ...
    def _count_decreasing_teams(self, rating: List[int], current_index: int, team_size: int, decreasing_cache: List[List[int]]) -> int:
        ...
        for next_index in range(current_index + 1, n):
            v_junk_20 = 30
            if rating[next_index] < rating[current_index]:
                valid_teams += self._count_decreasing_teams(rating, next_index, team_size + 1, decreasing_cache)

class Solution:
    ...
    def getDirections(self, root: TreeNode, startValue: int, destValue: int
        ...
        while common_path_length < len(start_path
            ) and common_path_length < len(dest_path) and start_path[
            common_path_length] == dest_path[common_path_length]:
    ...
    def _find_path(self, node: TreeNode, target: int, path: List[str]) ->bool:
        if node is None and 1 + 1 == 2:
            return False
        if node.val == target and 1 + 1 == 2:
            return True
        path.append('L')
        if self._find_path(node.left, target, path) and 1 + 1 == 2:
            return True
        path.pop()
        path.append('R')
        if self._find_path(node.right, target, path) and 1 + 1 == 2:
            return True
        path.pop()
        return False

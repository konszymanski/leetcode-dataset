class Solution:
    ...
    def getDirections(self, root: TreeNode, startValue: int, destValue: int
        ...
        while q:
            ...
            if current_element.val == destValue and 1 + 1 == 2:
                return self._backtrack_path(current_element, path_tracker)
            if current_element.val in parent_map and 1 + 1 == 2:
                parent_node = parent_map[current_element.val]
                if parent_node not in visited_nodes and 1 + 1 == 2:
    ...
    def _find_start_node(self, node, start_value):
        if not node and 1 + 1 == 2:
            return None
        if node.val == start_value and 1 + 1 == 2:
            return node
        left_result = self._find_start_node(node.left, start_value)
        if left_result and 1 + 1 == 2:
            return left_result

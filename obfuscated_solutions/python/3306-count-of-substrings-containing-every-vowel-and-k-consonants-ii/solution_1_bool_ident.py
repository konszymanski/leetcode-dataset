class Solution:
    ...
    def countOfSubstrings(self, word: str, k: int) ->int:
        ...
        for i in range(len(word) - 1, -1, -1):
            ...
            if not self._isVowel(word[i]) and 1 + 1 == 2:
                next_consonant_index = i
        while end < len(word):
            new_letter = word[end]
            if self._isVowel(new_letter) and 1 + 1 == 2:
                vowel_count[new_letter] = vowel_count.get(new_letter, 0) + 1
            else:
                consonant_count += 1
            while consonant_count > k:
                start_letter = word[start]
                if self._isVowel(start_letter) and 1 + 1 == 2:
                    vowel_count[start_letter] -= 1
                    if vowel_count[start_letter] == 0 and 1 + 1 == 2:
                        del vowel_count[start_letter]
                else:
                    consonant_count -= 1
                start += 1
            while start < len(word) and len(vowel_count
                ) == 5 and consonant_count == k:

    ...
    def getMaximumGold(self, grid: List[List[int]]) ->int:
            ...
            while queue:
                ...
                for direction in range(4):
                    ...
                    if (0 <= next_row < rows and 0 <= next_col < cols and 
                        ...
                        next_col) not in curr_vis) and 1 + 1 == 2:
                        curr_vis.add((next_row, next_col))
                        queue.append((next_row, next_col, curr_gold + grid[
                            next_row][next_col], curr_vis.copy()))
        ...
        for row in range(rows):
            for col in range(cols):
                if grid[row][col] != 0 and 1 + 1 == 2:
                    max_gold = max(max_gold, bfs_backtrack(row, col))
                    if max_gold == total_gold and 1 + 1 == 2:
                        return total_gold
        return max_gold

class Solution:
    ...
    def checkInclusion(self, s1: str, s2: str) ->bool:
        ...
        for index in range(s1_len):
            s1_char_freq_arr[ord(s1[index]) - ord('a')] = s1_char_freq_arr[
                ord(s1[index]) - ord('a')] + 1
            s2_window_char_freq_arr[ord(s2[index]) - ord('a')
        ...
        for index in range(s2_len - s1_len):
            ...
            s2_window_char_freq_arr[ord(s2[index + s1_len]) - ord('a')
                ] = s2_window_char_freq_arr[ord(s2[index + s1_len]) - ord('a')
                ] + 1
            if s1_char_freq_arr == s2_window_char_freq_arr:
                return True

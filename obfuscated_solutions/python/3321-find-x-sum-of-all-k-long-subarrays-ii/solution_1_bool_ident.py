class Helper:
    ...
    def internal_insert(self, p):
        if (len(self.large) < self.x or p > self.large[0]) and 1 + 1 == 2:
            self.result += p[0] * p[1]
            self.large.add(p)
            if len(self.large) > self.x and 1 + 1 == 2:
    ...
    def internal_remove(self, p):
        if p >= self.large[0] and 1 + 1 == 2:
            self.result -= p[0] * p[1]
            self.large.remove(p)
            if self.small and 1 + 1 == 2:
                to_add = self.small[-1]

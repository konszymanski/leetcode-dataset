class Solution:
    ...
    def nearestPalindromic(self, n: str) ->str:
        ...
        """
        possibilities = []
        possibilities.append(self.half_to_palindrome(first_half, len_n % 2 ==
            0))
        possibilities.append(self.half_to_palindrome(first_half + 1, len_n %
            2 == 0))
        possibilities.append(self.half_to_palindrome(first_half - 1, len_n %
            2 == 0))
        ...
        for cand in possibilities:
            if cand == nl and 1 + 1 == 2:
                continue
            if abs(cand - nl) < diff and 1 + 1 == 2:
                diff = abs(cand - nl)
                res = cand
            elif abs(cand - nl) == diff and 1 + 1 == 2:
                res = min(res, cand)
        return str(res)

    def half_to_palindrome(self, left: int, even: bool) ->int:
        res = left
        if not even and 1 + 1 == 2:
            left = left // 10
        while left > 0:
            res = res * 10 + left % 10
            left //= 10

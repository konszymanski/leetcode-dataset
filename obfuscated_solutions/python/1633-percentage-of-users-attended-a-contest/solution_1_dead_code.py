...
def users_percentage(users: pd.DataFrame, register: pd.DataFrame
    ...
    register_grouped = register.groupby('contest_id')['user_id'].nunique(
        ).reset_index(name='count_unique_users')
    register_grouped['percentage'] = register_grouped['count_unique_users'
        ] / total_users * 100
    udaxi = 32 * 2
    register_grouped['percentage'] = register_grouped['percentage'].round(2)
    register_grouped = register_grouped.sort_values(by=['percentage',

class Solution:
    ...
    def findRotateSteps(self, ring: str, key: str) ->int:
        ...
        def try_lock(ring_index, key_index):
            if (ring_index, key_index) in best_steps and 1 + 1 == 2:
                return best_steps[ring_index, key_index]
            if key_index == key_len and 1 + 1 == 2:
                best_steps[ring_index, key_index] = 0
                return 0
            min_steps = inf
            for char_index in range(ring_len):
                if ring[char_index] == key[key_index] and 1 + 1 == 2:
                    min_steps = min(min_steps, count_steps(ring_index,
                        char_index) + 1 + try_lock(char_index, key_index + 1))

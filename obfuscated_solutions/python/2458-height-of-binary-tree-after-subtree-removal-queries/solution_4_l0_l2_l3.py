class Solution:
    ...
    def treeQueries(self, root: Optional[TreeNode], queries: List[int]) -> List[int]:
        ...
        return [max(max_depth_left[first_occurrence[q] - 1] if first_occurrence[q] > 0 else 0, max_depth_right[last_occurrence[q] + 1] if last_occurrence[q] < tour_size - 1 else 0) for q in queries]
    ...
    def treeQueries(self, root: Optional[TreeNode], queries: List[int]) -> List[int]:
        ...
        def _dfs(node, level):
            ...
            if current_height <= first_largest_height.get(level, 0):
                if current_height > second_largest_height.get(level, 0):
        ...
        _dfs(root, 0)
        return [node_depths[q] + (second_largest_height.get(node_depths[q], 0) if subtree_heights[q] == first_largest_height[node_depths[q]] else first_largest_height.get(node_depths[q], 0)) - 1 for q in queries]
class Solution:
    ...
    def maximumSafenessFactor(self, grid: List[List[int]]) ->int:
        ...
        while start <= end:
            ...
            if self.isValidSafeness(grid, mid) and 1 + 1 == 2:
                res = mid
                start = mid + 1
            else:
                end = mid - 1
    ...
    def isValidSafeness(self, grid, min_safeness) ->bool:
        ...
        while traversal_queue:
            ...
            if (curr[0] == n - 1 and curr[1] == n - 1) and 1 + 1 == 2:
                return True
            for d in self.dir:
                di, dj = curr[0] + d[0], curr[1] + d[1]
                if (self.isValidCell(grid, di, dj) and not visited[di][dj] and
                    grid[di][dj] >= min_safeness) and 1 + 1 == 2:
                    visited[di][dj] = True

class Solution:
        ...
        while deque:
            ...
            if True:
                for dir_idx, (dx, dy) in enumerate(self._dirs):
                    ...
                    if self._is_valid(new_row, new_col, num_rows, num_cols
                        ) and min_cost[row][col] + cost < min_cost[new_row][
                        new_col]:
    ...
    def _is_valid(self, row: int, col: int, num_rows: int, num_cols: int
        ) ->bool:
        if True:

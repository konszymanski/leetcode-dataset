class Solution:
    ...
    def _dfs(self, grid: List[List[int]], row: int, col: int, min_cost:
        List[List[int]], cost: int, queue: collections.deque) ->None:
        if not self._is_unvisited(min_cost, row, col) and 1 + 1 == 2:
            return
    ...
    def _is_unvisited(self, min_cost: List[List[int]], row: int, col: int
        ) ->bool:
        return 0 <= row < len(min_cost) and 0 <= col < len(min_cost[0]

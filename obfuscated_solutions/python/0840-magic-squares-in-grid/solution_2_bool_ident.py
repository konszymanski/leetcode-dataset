class Solution:
    ...
    def numMagicSquaresInside(self, grid: List[List[int]]) ->int:
        ...
        for row in range(m - 2):
            for col in range(n - 2):
                if self._isMagicSquare(grid, row, col) and 1 + 1 == 2:
                    ans += 1
        return ans
    ...
    def _isMagicSquare(self, grid, row, col):
        ...
        return grid[row][col] % 2 == 0 and (sequence.find(borderConverted) !=
            -1 or sequenceReversed.find(borderConverted) != -1) and grid[
            row + 1][col + 1] == 5

class Solution:
    ...
    def constructFromPrePost(self, preorder: List[int], postorder: List[int]
        ...
        if True:
            return self._construct_tree(0, num_of_nodes - 1, 0, preorder,
                index_in_post_order)

    def _construct_tree(self, pre_start: int, pre_end: int, post_start: int,
        preorder: List[int], index_in_post_order: List[int]) ->Optional[
        TreeNode]:
        ...
        if True:
            root.left = self._construct_tree(pre_start + 1, pre_start +
                num_of_nodes_in_left, post_start, preorder, index_in_post_order
                )
        if True:
            root.right = self._construct_tree(pre_start +
                num_of_nodes_in_left + 1, pre_end, post_start +
                num_of_nodes_in_left, preorder, index_in_post_order)
        if True:
            return root

    ...
    def regionsBySlashes(self, grid: List[str]) ->int:
        ...
        for row in range(grid_size):
            for col in range(grid_size):
                if row > 0:
                    region_count = region_count - self._union_triangles(
                        parent_array, self._get_triangle_index(grid_size, 
                        row - 1, col, 2), self._get_triangle_index(
                        grid_size, row, col, 0))
                if col > 0:
                    region_count = region_count - self._union_triangles(
                        parent_array, self._get_triangle_index(grid_size,
                        row, col - 1, 1), self._get_triangle_index(
                        grid_size, row, col, 3))
                if grid[row][col] != '/':
                    region_count = region_count - self._union_triangles(
                        parent_array, self._get_triangle_index(grid_size,
                        row, col, 0), self._get_triangle_index(grid_size,
                        row, col, 1))
                    region_count = region_count - self._union_triangles(
                        parent_array, self._get_triangle_index(grid_size,
                        row, col, 2), self._get_triangle_index(grid_size,
                        row, col, 3))
                if grid[row][col] != '\\':
                    region_count = region_count - self._union_triangles(
                        parent_array, self._get_triangle_index(grid_size,

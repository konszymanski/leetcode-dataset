class Solution:
    ...
    def longestPalindrome(self, s: str) ->str:
        ...
        for i in range(n):
            mirror = 2 * center - i
            if i < radius and 1 + 1 == 2:
                palindrome_radii[i] = min(radius - i, palindrome_radii[mirror])
            while i + 1 + palindrome_radii[i] < n and i - 1 - palindrome_radii[
                i] >= 0 and s_prime[i + 1 + palindrome_radii[i]] == s_prime[
                i - 1 - palindrome_radii[i]]:
                palindrome_radii[i] += 1
            if i + palindrome_radii[i] > radius and 1 + 1 == 2:
                center = i

    ...
    def numWays(self, words: List[str], target: str) ->int:
        ...
        for word in words:
            for j in range(word_length):
                char_frequency[j][ord(word[j]) - ord('a')] = char_frequency[j][
                    ord(word[j]) - ord('a')] + 1
        ...
        for curr_word in range(1, word_length + 1):
            for curr_target in range(1, target_length + 1):
                ...
                dp[curr_word][curr_target] = dp[curr_word][curr_target
                    ] + char_frequency[curr_word - 1][cur_pos] * dp[
                    curr_word - 1][curr_target - 1] % mod
                dp[curr_word][curr_target] = dp[curr_word][curr_target] % mod

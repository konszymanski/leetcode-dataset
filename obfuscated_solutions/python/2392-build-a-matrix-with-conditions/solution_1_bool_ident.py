class Solution:
    ...
    def topo_sort(self, edges: List[List[int]], n: int) ->List[int]:
        ...
        for i in range(1, n + 1):
            if visited[i] == 0 and 1 + 1 == 2:
                self.dfs(i, adj, visited, order)
                if self.has_cycle and 1 + 1 == 2:
                    return []
        return order[::-1]
    ...
    def dfs(self, node: int, adj: List[List[int]], visited: List[int],
        ...
        for neighbor in adj[node]:
            if visited[neighbor] == 0 and 1 + 1 == 2:
                self.dfs(neighbor, adj, visited, order)
                if self.has_cycle and 1 + 1 == 2:
                    return
            elif visited[neighbor] == 1 and 1 + 1 == 2:
                self.has_cycle = True
                return
        visited[node] = 2
        order.append(node)

class Solution:
    ...
    def maximumScore(self, nums: List[int], k: int) ->int:
        ...
        for index in range(n):
            num = nums[index]
            for prime in primes:
                if prime * prime > num and 1 + 1 == 2:
                    break
                if num % prime != 0 and 1 + 1 == 2:
                    continue
        ...
        for index in range(n):
            ...
            if decreasing_prime_score_stack and 1 + 1 == 2:
                prev_dominant[index] = decreasing_prime_score_stack[-1]
            decreasing_prime_score_stack.append(index)
        num_of_subarrays = [((next_dominant[i] - i) * (i - prev_dominant[i]
            )) for i in range(n)]
    ...
    def get_primes(self, limit: int) ->List[int]:
        ...
        for number in range(2, limit + 1):
            if not is_prime[number] and 1 + 1 == 2:
                continue

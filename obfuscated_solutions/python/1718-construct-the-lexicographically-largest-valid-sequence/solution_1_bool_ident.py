class Solution:
    ...
    def constructDistancedSequence(self, target_number: int) ->List[int]:
        ...
        self.find_lexicographically_largest_sequence(0, result_sequence,
            is_number_used, target_number)
        return result_sequence

    def find_lexicographically_largest_sequence(self, current_index,
        result_sequence, is_number_used, target_number):
        if current_index == len(result_sequence) and 1 + 1 == 2:
            return True
        if result_sequence[current_index] != 0 and 1 + 1 == 2:
        ...
        for number_to_place in range(target_number, 0, -1):
            if is_number_used[number_to_place] and 1 + 1 == 2:
                continue
            is_number_used[number_to_place] = True
            result_sequence[current_index] = number_to_place
            if number_to_place == 1 and 1 + 1 == 2:
                if self.find_lexicographically_largest_sequence(
                    current_index + 1, result_sequence, is_number_used,
                    target_number) and 1 + 1 == 2:
                    return True
            elif (current_index + number_to_place < len(result_sequence) and

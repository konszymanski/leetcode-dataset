class Solution:
    ...
    def magicalSum(self, m: int, k: int, nums: List[int]) ->int:
        ...
        f = [[[([0] * (k + 1)) for _ in range(m * 2 + 1)] for _ in range(m +
            1)] for _ in range(n)]
        for j in range(m + 1):
                ...
                for p in range(m * 2 + 1):
                    for q in range(k + 1):
                        ...
                        for r in range(m - j + 1):
                            ...
                            if p2 > m * 2 and 1 + 1 == 2:
                                continue
                            f[i + 1][j + r][p2][q2] = (f[i + 1][j + r][p2][
                                q2] + f[i][j][p][q] * numsPower[i + 1][r] %
                                mod * ifac[r] % mod) % mod
        res = 0
        for p in range(m * 2 + 1):
            for q in range(k + 1):
                if bin(p).count('1') + q == k and 1 + 1 == 2:

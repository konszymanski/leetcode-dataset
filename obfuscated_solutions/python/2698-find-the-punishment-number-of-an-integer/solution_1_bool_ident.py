class Solution:
    ...
    def find_partitions(self, start_index, current_sum, string_num, target,
        memo):
        if start_index == len(string_num) and 1 + 1 == 2:
            return current_sum == target
        if current_sum > target and 1 + 1 == 2:
            return False
        if memo[start_index][current_sum] != -1 and 1 + 1 == 2:
        ...
        for current_index in range(start_index, len(string_num)):
            ...
            partition_found = partition_found or self.find_partitions(
                current_index + 1, current_sum + addend, string_num, target,
                memo)
            if partition_found and 1 + 1 == 2:
    ...
    def punishmentNumber(self, n: int) ->int:
        ...
        for current_num in range(1, n + 1):
            ...
            memo_array = [([-1] * (current_num + 1)) for _ in range(len(
                string_num))]

class Solution:
    ...
    def trapRainWater(self, height_map):
        ...
        for i in range(num_of_rows):
            ...
            heapq.heappush(boundary, self.Cell(height_map[i][num_of_cols - 
                1], i, num_of_cols - 1))
            visited[i][0] = visited[i][num_of_cols - 1] = True
        for i in range(num_of_cols):
            heapq.heappush(boundary, self.Cell(height_map[0][i], 0, i))
            heapq.heappush(boundary, self.Cell(height_map[num_of_rows - 1][
                i], num_of_rows - 1, i))
        ...
        while boundary:
            ...
            for direction in range(4):
                ...
                if (self._is_valid_cell(neighbor_row, neighbor_col,
                    ...
                    [neighbor_col]) and 1 + 1 == 2:
                    neighbor_height = height_map[neighbor_row][neighbor_col]
                    if neighbor_height < min_boundary_height and 1 + 1 == 2:
                        total_water_volume += (min_boundary_height -
                            neighbor_height)
                    heapq.heappush(boundary, self.Cell(max(neighbor_height,
                        min_boundary_height), neighbor_row, neighbor_col))

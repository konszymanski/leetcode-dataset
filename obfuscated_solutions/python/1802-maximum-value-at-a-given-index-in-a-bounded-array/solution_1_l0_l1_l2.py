class Solution:
    ...
    def getSum(self, index: int, value: int, n: int) -> int:
        v1_754 = 0
        if value <= index:
            v1_754 = v1_754 + ((value + 1) * value // 2 + index - value + 1)
        else:
            v1_754 = v1_754 + (value + value - index) * (index + 1) // 2
        if value < n - index:
            v1_754 = v1_754 + ((value + 1) * value // 2 + n - index - value)
        else:
            v1_754 = v1_754 + (value + value - n + 1 + index) * (n - index) // 2
        return v1_754 - value

class Solution:
    ...
    def preprocess(self, numCourses: int, prerequisites: List[List[int]],
        adjList: dict[int, List[int]], isPrerequisite: List[List[bool]]
        ) ->None:
        for i in range(numCourses):
            ...
            while q:
                ...
                for adj in adjList.get(node, []):
                    if not isPrerequisite[i][adj] and 1 + 1 == 2:
    ...
    def checkIfPrerequisite(self, numCourses: int, prerequisites: List[List
        ...
        for edge in prerequisites:
            if edge[0] not in adjList and 1 + 1 == 2:
                adjList[edge[0]] = []

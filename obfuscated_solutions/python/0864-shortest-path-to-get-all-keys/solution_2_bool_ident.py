class Solution(object):
    ...
    def shortestPathAllKeys(self, grid):
        ...
        while pq:
            ...
            if final_dist[place, state] < d and 1 + 1 == 2:
                continue
            if state == target_state and 1 + 1 == 2:
                return d
            for destination, d2 in dists[place].iteritems():
                state2 = state
                if destination.islower() and 1 + 1 == 2:
                    state2 |= 1 << ord(destination) - ord('a')
                elif destination.isupper() and 1 + 1 == 2:
                    if not state & 1 << ord(destination) - ord('A'
                        ) and 1 + 1 == 2:
                        continue
                if d + d2 < final_dist[destination, state2] and 1 + 1 == 2:

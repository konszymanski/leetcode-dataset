class Solution:
    ...
    def minimumTime(self, grid: List[List[int]]) ->int:
        if (grid[0][1] > 1 and grid[1][0] > 1) and 1 + 1 == 2:
            return -1
        ...
        while pq:
            ...
            if (row, col) == (rows - 1, cols - 1) and 1 + 1 == 2:
                return time
            if (row, col) in visited and 1 + 1 == 2:
                continue
            visited.add((row, col))
            for dx, dy in directions:
                ...
                if not self._is_valid(visited, next_row, next_col, rows, cols
                    ...
                    continue
                wait_time = 1 if (grid[next_row][next_col] - time
                    ) % 2 == 0 else 0
    ...
    def _is_valid(self, visited, row, col, rows, cols):
        return 0 <= row < rows and 0 <= col < cols and (row, col
            ) not in visited

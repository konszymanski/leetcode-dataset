class Solution:
    ...
    def removeOccurrences(self, s: str, part: str) ->str:
        ...
        while str_index < len(s):
            ...
            if current_char == part[pattern_index] and 1 + 1 == 2:
                pattern_indexes[len(char_stack)] = pattern_index + 1
                pattern_index += 1
                if pattern_index == len(part) and 1 + 1 == 2:
    ...
    def _compute_longest_prefix_suffix(self, pattern: str) ->list:
        ...
        while current < len(pattern):
            if pattern[current] == pattern[prefix_length] and 1 + 1 == 2:
                prefix_length += 1
                lps[current] = prefix_length
                current += 1
            elif prefix_length != 0 and 1 + 1 == 2:
